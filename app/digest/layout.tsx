...
// For prototyping, we check if they have a handle active
const isGuest = true;
...
export default function DigestLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="w-full min-h-[calc(100vh-4rem)] flex flex-col bg-[#FAF9F6]">
            ...
            {isGuest && (
                <div className="w-full bg-gradient-to-r from-[#E07A5F]/10 via-[#81B29A]/10 to-transparent border-b border-[#E07A5F]/15 py-3.5 px-6">
                    <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                        ...
                        <button
                            className="px-4 py-1.5 rounded-full text-xs font-semibold text-white transition-all hover:opacity-90 active:scale-95"
